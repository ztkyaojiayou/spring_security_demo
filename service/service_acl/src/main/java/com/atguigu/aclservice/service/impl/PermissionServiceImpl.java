...
//        });
    ...
    public List<String> getPermissionValueByUserId(String id) {
        ...
        if(this.isSysAdmin(id)) {
            //如果是系统管理员，则获取所有权限
            permissionList = baseMapper.selectAllPermissionValue();
        } else {
            permissionList = baseMapper.selectPermissionValueByUserId(id);
        }
        return permissionList;
    }
    ...
    public void saveRolePermissionRealtionShipGuli(String roleId, String[] permissionIds) {
        //注意：这里就不像“给用户分配角色”一样，这里并没有先删除当前角色的所有权限后再查询添加！！！

        //roleId角色id
        //permissionId菜单id 数组形式
        //1 创建list集合，用于封装添加数据，以便批量新增
        List<RolePermission> rolePermissionList = new ArrayList<>();
        //遍历所有菜单数组
